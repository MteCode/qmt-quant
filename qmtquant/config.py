...
class CostConfig:
    ...
    #: 滑点（相对值，单边）。回测引擎用这个。
    #:
    #: 为什么回测不用 tick：本地行情是**后复权价**，且锚在 IPO，
    #: 复权因子从 1.6（工行）到 104（平安银行）不等。把 0.01 元加到
    #: 后复权价上，对平安银行只滑了真实价的百分之一个 tick。
    #: 要按 tick 换算就得知道真实价，而 turnover/volume 的单位
    #: （股还是手）在 BarData 契约里没有规定 —— 恰恰在因子最大的标的上
    #: 两种解释都落在"合理"区间，猜不出来。
    #:
    #: 相对滑点没有这个问题：同一个比例施加在后复权价和真实价上
    #: 效果相同，既不需要因子也不需要单位。
    slippage_rate: float = 0.0005       # 万 5，单边
    #: 滑点（按最小变动价位）。模拟撮合网关用这个 —— 它拿到的是
    #: 实时行情的**真实价**，不存在复权问题，按 tick 更贴近真实盘口。
    slippage_tick: int = 1
