...
费率从 qmtquant.core.costs 取，不在这里写死。当前口径：
佣金万 0.854（**最低 5 元/笔**）+ 印花税 0.05%（仅卖出，2023-08-28
起减半）+ 过户费万 0.1（双边）+ 滑点万 5（单边），名义往返 0.169%。

但名义值有误导性：最低佣金让单笔低于 58,548 元时实际费率更高 ——
单笔 2.5 万时往返 0.192%，1 万时 0.252%。做 T 天然把资金拆成小单，
几乎全落在这一段，所以 net_edge 必须按**真实平均单笔金额**算。

此处早期版本写的是「0.25%（印花税 0.10%）」，那是减半前的旧值。
